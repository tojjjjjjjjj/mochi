...
export default function MochiCard({ mochi }: MochiCardProps) {
  ...
  return (
    <Link href={`/mochi/${mochi.slug}`} className="block" style={{ textDecoration: "none" }}>
      <div className="mochi-card" style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        {/* Hero */}
        <div style={{ height: 140, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, position: "relative", background: heroBg }}>
          <button onClick={handleBookmark} aria-label={saved ? "Remove from Lunchbox" : "Save to Lunchbox"} className="cursor-pointer"
            style={{ position: "absolute", top: 10, left: 10, width: 32, height: 32, borderRadius: 8, background: "rgba(255,255,255,0.85)", backdropFilter: "blur(8px)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, zIndex: 2 }}>
            {saved ? "\u2764\uFE0F" : "\uD83E\uDD0D"}
          </button>
          <span aria-hidden="true">{mochi.flavor_emoji}</span>
          <span style={{ position: "absolute", top: 12, right: 12, padding: "4px 12px", borderRadius: 100, fontSize: 12, fontWeight: 700, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(8px)", color: mochi.is_house_special ? "var(--warning)" : "var(--success)" }}>
        ...
        <div style={{ padding: "16px 20px 18px", flex: 1, display: "flex", flexDirection: "column" }}>
          <h3 style={{ fontWeight: 700, fontSize: 18, letterSpacing: "-0.02em", lineHeight: 1.3, color: "var(--t1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 4 }}>
            {mochi.title}
          </h3>
          <p style={{ fontSize: 14, color: "var(--t2)", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: 1.45, marginBottom: 14 }}>
            {mochi.tagline}
          </p>
          ...
          <div className="flex items-center justify-between" style={{ marginTop: "auto" }}>
            <div className="flex items-center" style={{ gap: 4 }}>
              {compatibleAgents.map((agent) => (
                <span key={agent.id} title={agent.name} aria-label={agent.name}
                  style={{ width: 22, height: 22, borderRadius: 6, backgroundColor: agent.squareColor, color: "#FFF", fontSize: 10, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>
                  {agent.letter}
                </span>
              ))}
        ...
        {/* Feed button */}
        <button onClick={handleFeed} className="feed-btn btn-bouncy cursor-pointer"
          aria-label={`Copy ${mochi.type === "prompt" ? "prompt" : "skill"}: ${mochi.title}`}
          style={{ position: "absolute", bottom: 16, right: 16, padding: "8px 16px", minHeight: 36, fontSize: 13 }}>
          Feed
        </button>
      </div>
    </Link>
